...
ADMIN_TOKEN = f'@{ADMIN_PHONE}'  # Admin login token: @7900012929
...
@app.route('/admin/quicklogin')
def admin_quicklogin():
    """Quick access for local testing"""
    session['admin'] = True
    session['admin_user'] = 'admin'
    flash('Admin access granted (quicklogin)', 'success')
    return redirect(url_for('admin_dashboard'))


@app.route('/admin/forcelogin')
def admin_forcelogin():
    """TEMPORARY: Force set admin session (for local testing only - remove in production)"""
    session['admin'] = True
    session['admin_user'] = 'admin'
    flash('Forced admin login (DEBUG)', 'warning')
    return redirect(url_for('admin_dashboard'))
